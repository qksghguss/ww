...
function getUnitsPerBox(item: Item) {
  return item.unitsPerBox && item.unitsPerBox > 0 ? item.unitsPerBox : 1;
}

function convertLineItemToEach(line: RequestLineItem, item: Item) {
  const unitsPerBox = getUnitsPerBox(item);
  return line.unit === 'box' ? line.quantity * unitsPerBox : line.quantity;
}

function applyInventoryByRequest(
  items: Item[],
  lineItems: RequestLineItem[],
  direction: 1 | -1
) {
  if (lineItems.length === 0) {
    return { items, breakdown: [] as InventoryBreakdown[] };
  }
  ...
  for (const line of lineItems) {
    const item = items.find((candidate) => candidate.id === line.itemId);
    if (!item) continue;
    const eachQuantity = convertLineItemToEach(line, item);
    deltaMap.set(item.id, (deltaMap.get(item.id) ?? 0) + eachQuantity * direction);
  }
  ...
  const updatedItems = items.map((item) => {
    ...
    const unitsPerBox = getUnitsPerBox(item);
    const boxes = item.unitsPerBox ? Math.floor(nextStock / unitsPerBox) : undefined;
    const remainder = item.unitsPerBox ? nextStock % unitsPerBox : undefined;
    breakdown.push({
      itemId: item.id,
      name: item.name,
      remainingEach: nextStock,
  ...
  return { items: updatedItems, breakdown };
}
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'UPSERT_ISSUE_REQUEST': {
      ...
      let itemsAfterChange = state.items;
      let breakdown: InventoryBreakdown[] = [];

      if (existing?.status === 'fulfilled') {
        const revert = applyInventoryByRequest(itemsAfterChange, existing.lineItems, 1);
        itemsAfterChange = revert.items;
        if (withTimestamps.status !== 'fulfilled') {
          breakdown = revert.breakdown;
        }
      }

      if (withTimestamps.status === 'fulfilled') {
        const apply = applyInventoryByRequest(itemsAfterChange, withTimestamps.lineItems, -1);
        itemsAfterChange = apply.items;
        breakdown = apply.breakdown;
      }
      ...
      const audit: AuditLog = {
        ...
        meta: {
          ...
          items: issueItemNames,
          inventory:
            breakdown.length > 0
              ? breakdown.map((entry) => ({
                  itemId: entry.itemId,
                  name: entry.name,
                  remainingEach: entry.remainingEach,
                  remainingBoxes: entry.remainingBoxes,
                  remainingRemainder: entry.remainingRemainder,
                  deltaEach: entry.deltaEach
                }))
              : undefined
        }
      };
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const attemptLoad = async (attempt = 0) => {
      ...
      setIsSyncing(true);
      try {
        const { state: loadedState, source } = await loadDataState();
        if (cancelled) return;
...
export function calculateTotalQuantity(
  ...
  return lineItems.reduce((total, line) => {
    ...
    const unitsPerBox = getUnitsPerBox(item);
    return total + eachQuantity / unitsPerBox;
  }, 0);
}
