...
import { downloadFile, formatStockSummary } from '../lib/utils';
import { useDebounce } from '../hooks/use-debounce';
import type { Item } from '../types';

export function InventoryPage() {
  const { items, adjustInventory } = useData();
  const { user } = useAuth();
  ...
  const selectedLatest = selectedItem
    ? items.find((item) => item.id === selectedItem.id) ?? selectedItem
    : null;
  const selectedThresholdSummary = selectedLatest
    ? selectedLatest.unitsPerBox
      ? formatStockSummary({ ...selectedLatest, stock: selectedLatest.threshold })
      : `${selectedLatest.threshold}낱개`
    : '';

  const handleAdjust = () => {
    if (!selectedItem || !user || adjustment === 0) {
      if (adjustment === 0) {
        toast.error('조정 수량을 입력하세요.');
      }
      return;
    }
    const latest = items.find((item) => item.id === selectedItem.id) ?? selectedItem;
    const newStock = Math.max(0, latest.stock + adjustment);
    const note = adjustment > 0 ? `+${adjustment}` : `${adjustment}`;
    adjustInventory(latest.id, newStock, user.id, note);
  ...
  const handleExport = () => {
    const csv = Papa.unparse({
      ...
      data: items.map((item) => [
        ...
        item.threshold,
        item.category,
        item.stock <= item.threshold ? '임계치 이하' : ''
      ])
    });
          ...
          <div className="flex flex-wrap items-center gap-2">
            {lowStockItems.map((item) => (
              <span key={item.id} className="rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-700">
                {item.name} 저재고 ({formatStockSummary(item)})
              </span>
            ))}
          </div>
        </div>
                        ...
                        <div className="space-y-4">
                          {selectedLatest && (
                            <div className="space-y-1 rounded-md bg-slate-50 p-3 text-sm text-slate-600">
                              <p>현재 재고: {formatStockSummary(selectedLatest)}</p>
                              <p>임계치: {selectedThresholdSummary}</p>
                              <p className="text-xs text-slate-500">조정 수량은 낱개 기준으로 입력하세요.</p>
                            </div>
                          )}
