...
import { downloadFile, formatDate, formatRequestLineSummary } from '../lib/utils';
...
export function PurchaseRequestsPage() {
  const { items, purchaseRequests, upsertPurchaseRequest, removePurchaseRequest, users } = useData();
  const { user } = useAuth();
  ...
  const handleExport = () => {
    const csv = Papa.unparse({
      fields: ['요청ID', '요청자', '상태', '요청일', '최종수정일', '요청 품목', '메모', '첨부파일'],
    ...
    downloadFile('purchase-requests.csv', csv);
    toast.success('구매 요청 데이터를 CSV로 내보냈습니다.');
  };
  ...
  const handleCreateRequest = (status: PurchaseRequest['status']) => {
    ...
    const existing = editingRequestId
      ? purchaseRequests.find((req) => req.id === editingRequestId)
      : undefined;
    ...
    toast.success(editingRequestId ? '구매 요청이 업데이트되었습니다.' : '구매 요청이 생성되었습니다.');
    resetForm();
  };

  const handleLoadRequest = (request: PurchaseRequest) => {
    ...
    toast.info('구매 요청 내용을 편집 폼에 불러왔습니다.');
  };
  ...
  const handleDeleteRequest = (request: PurchaseRequest) => {
    ...
    if (user.role !== 'admin' && request.status === 'ordered') {
      toast.error('발주 완료된 요청은 삭제할 수 없습니다.');
      return;
    }
    if (!confirm('선택한 구매 요청을 삭제하시겠습니까?')) return;
    removePurchaseRequest(request.id, user.id);
                  ...
                  <TableRow key={request.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex flex-wrap justify-end gap-2">
                        ...
                        {(user?.role === 'admin' ||
                          (user?.id === request.requestedBy && request.status !== 'ordered')) && (
                          <Button
                            size="sm"
                            variant="destructive"
