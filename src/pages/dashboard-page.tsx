...
import { formatDate, convertRequestLineToEach, formatStockSummary } from '../lib/utils';
...
export function DashboardPage() {
  const { items, issueRequests, purchaseRequests, users, auditLogs } = useData();
  const { user } = useAuth();
  ...
  const fulfilledIssueRequests = useMemo(
    () => issueRequests.filter((request) => request.status === 'fulfilled'),
    [issueRequests]
  );

  const completedPurchaseRequests = useMemo(
    () => purchaseRequests.filter((request) => request.status === 'ordered'),
    [purchaseRequests]
  );
  ...
  const topUsage = useMemo(() => {
    ...
    fulfilledIssueRequests.forEach((request) => {
      request.lineItems.forEach((line) => {
        const item = itemMap.get(line.itemId);
        usageMap.set(
          line.itemId,
          (usageMap.get(line.itemId) ?? 0) + convertRequestLineToEach(line, item)
        );
      });
    });
    return items
      ...
      .slice(0, 5);
  }, [fulfilledIssueRequests, itemMap, items]);

  const monthlyTrendData = useMemo(() => {
    const totals = new Map<string, { issue: number; purchase: number }>();

    fulfilledIssueRequests.forEach((request) => {
      const month = request.createdAt.slice(0, 7);
      const existing = totals.get(month) ?? { issue: 0, purchase: 0 };
      existing.issue += request.lineItems.reduce(
        (acc, line) => acc + convertRequestLineToEach(line, itemMap.get(line.itemId)),
        0
      );
      totals.set(month, existing);
    });

    completedPurchaseRequests.forEach((request) => {
      const month = request.createdAt.slice(0, 7);
      const existing = totals.get(month) ?? { issue: 0, purchase: 0 };
      existing.purchase += request.lineItems.reduce(
        (acc, line) => acc + convertRequestLineToEach(line, itemMap.get(line.itemId)),
        0
      );
      totals.set(month, existing);
  ...
  }, [completedPurchaseRequests, fulfilledIssueRequests, itemMap]);

  const userDistribution = useMemo(() => {
    const counts = fulfilledIssueRequests.reduce<Record<string, number>>((acc, request) => {
      acc[request.requestedBy] =
        (acc[request.requestedBy] ?? 0) +
        request.lineItems.reduce(
          (sum, line) => sum + convertRequestLineToEach(line, itemMap.get(line.itemId)),
          0
        );
      return acc;
    }, {});
  ...
  }, [fulfilledIssueRequests, itemMap, users]);
  ...
  const recentActivities = useMemo(() => {
    const scopedLogs =
      user?.role === 'user'
        ? auditLogs.filter((log) => log.category === 'inventory')
        : auditLogs;
    return scopedLogs
      .slice(0, 6)
      .map((log) => ({
        id: log.id,
        description: log.target ? `${log.action} • ${log.target}` : log.action,
  ...
  const myIssueRequests = useMemo(() => {
    ...
    return issueRequests
      .filter((request) => request.requestedBy === user.id && request.status !== 'draft')
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>지급 완료 및 발주 완료 기준으로 월별 처리량을 확인하세요.</CardDescription>
          </CardHeader>
              ...
              {monthlyTrendData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyTrendData}>
                    <XAxis dataKey="month" />
                    <YAxis />
                    <Tooltip formatter={(value) => `${value as number}건`} />
                    <Bar dataKey="issue" name="지급" fill="#2563eb" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="purchase" name="구매" fill="#60a5fa" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </CardContent>
          ...
          <CardHeader>
            ...
            <CardDescription>지급 완료된 요청을 기준으로 요청 비중을 확인하세요.</CardDescription>
          </CardHeader>
            <CardContent className="h-80">
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>재고 관련 최신 로그를 확인하세요.</CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-3">
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <Card className="lg:col-span-2">
        <CardHeader>
          <CardTitle>월별 지급/구매 추세</CardTitle>
          <CardDescription>지급 완료 및 발주 완료 기준으로 지급과 구매 추세를 확인하세요.</CardDescription>
        </CardHeader>
        <CardContent className="h-80">
          {monthlyTrendData.length === 0 ? (
            <div className="flex h-full items-center justify-center text-sm text-slate-500">
      ...
      <Card>
        <CardHeader>
          <CardTitle>사용자별 요청 비중</CardTitle>
          <CardDescription>지급 완료된 요청을 기준으로 사용자별 비중을 확인하세요.</CardDescription>
        </CardHeader>
        <CardContent className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
      ...
      <Card className="lg:col-span-2">
        <CardHeader>
          ...
          <CardDescription>지급 완료된 요청 기준 상위 5개 품목입니다.</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          {lowStock.map((item) => (
            <div key={item.id} className="rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <p className="text-xs text-amber-700">
                재고 {formatStockSummary(item)} / 임계치 {item.unitsPerBox
      ...
      <Card className="lg:col-span-3">
        <CardHeader>
          <CardTitle>최근 활동</CardTitle>
          <CardDescription>이력 관리와 실시간으로 동기화된 최신 작업 내역입니다.</CardDescription>
        </CardHeader>
        <CardContent>
          <ul className="space-y-3">
            {recentActivities.length === 0 && (
