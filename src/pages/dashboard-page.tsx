...
export function DashboardPage() {
  ...
  const monthlyTrendData = useMemo(() => {
    ...
    fulfilledIssueRequests.forEach((request) => {
      ...
      existing.issue += request.lineItems.reduce((acc, item) => acc + item.quantity, 0);
    ...
    completedPurchaseRequests.forEach((request) => {
      const month = request.createdAt.slice(0, 7);
      const existing = totals.get(month) ?? { issue: 0, purchase: 0 };
      existing.purchase += request.lineItems.reduce((acc, item) => acc + item.quantity, 0);
      totals.set(month, existing);
    });
  ...
  }, [completedPurchaseRequests, fulfilledIssueRequests]);
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>지급 완료된 요청 기준 상위 품목입니다.</CardDescription>
          </CardHeader>
          <CardContent>
