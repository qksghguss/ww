...
const navigation = [
  ...
  { name: '품목 관리', to: '/items', icon: Package },
  { name: '재고 관리', to: '/inventory', icon: ClipboardList },
  { name: '지급 요청', to: '/issue-requests', icon: ShoppingCart },
  { name: '구매 요청', to: '/purchase-requests', icon: ShoppingCart },
  { name: '이력 관리', to: '/history', icon: History }
];
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  if (user?.role === 'admin') {
    items.push({ name: '사용자 관리', to: '/users', icon: Users });
  }
